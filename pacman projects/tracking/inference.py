    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldP in self.legalPositions:
            ...
            for p in self.legalPositions:
                apost[p] += newPosDist[p]*self.beliefs[oldP]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for position in self.legalPositions:
        	for i in xrange(numParticlesPerPosition):
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
        	...
        	sampleControl = util.sample(newPosDist)
        	predictedParticles.append(sampleControl)
        self.particles = predictedParticles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        allPossComb = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(allPossComb)
        count = 0
        # numParticlesPerPosition = self.numParticles/len(allPossComb)
        # print self.numParticles, len(allPossComb)
        while count < self.numParticles:
        	for comb in allPossComb:
        		self.particles.append(comb)
        		count +=1
        # for comb in allPossComb:
        # 	for i in xrange(numParticlesPerPosition):
        # 		self.particles.append(comb)
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
        	w = 1
        	for gidx in xrange(self.numGhosts):
        		if noisyDistances[gidx] is None:
        			particle = self.getParticleWithGhostInJail(particle, gidx)
        		else:
        			w *= emissionModels[gidx][util.manhattanDistance(particle[gidx],pacmanPosition)]
        	weights[particle] += w

        if weights.totalCount() == 0:
        	self.initializeParticles()
        	for pidx in xrange(len(self.particles)):
        		for gidx in xrange(self.numGhosts):
        			if noisyDistances[gidx] is None:
        				self.particles[pidx] = self.getParticleWithGhostInJail(self.particles[pidx], gidx)
        else:
        	weights.normalize()
        	resampledParticles = []
        	for i in xrange(self.numParticles):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for gidx in xrange(self.numGhosts):
            	newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), gidx, self.ghostAgents[gidx])
            	sampleControl = util.sample(newPosDist)
            	newParticle[gidx] = sampleControl
